...
class faceAssigner():
    ...
    def __init__(self):
        ...
        # unassigned_1 = Face.objects.filter(unassigned_crit).order_by('?')[0]
        # print(unassigned_1)
        ...
        # self.classify_unassigned(unassigned_1)
    ...
    def classify_unassigned(self, u_img):
        ...
        if self.DEBUG:
            ...
            face_img = Image.open(u_img.face_thumbnail)
            face_thumbnail = np.asarray(face_img)
            face_img.close()
            source_img_file = u_img.source_image_file.filename
            roll_up_data = {
                'query_img': {
                    'person_name': u_img.declared_name.person_name,
                    'short_encoding': short_encoding,
                    'date': date_string,
                    'face_thumbnail': face_thumbnail,
                    'id': u_img.id,
                    'box_top': u_img.box_top,
                    'box_bottom': u_img.box_bottom,
                    'box_left': u_img.box_left,
                    'box_right': u_img.box_right,
        ...
        for row_num, known_id in enumerate(self.person_ids):
            ...
            if date < first_date_person and date > self.bogus_date_utc: 
                ...
                closest_faces = Face.objects.annotate(result=Abs(F('dateTakenUTC') - date)) \
                    .filter(faces_person & has_short).order_by('result')[:N_COMPARISONS]

                print('before enc_retrieval', time.time() - s)
                # Get the N_COMPARISONS closest faces
                # print(f'Retrieve 0: {time.time() - s}')

                s = time.time()
                short_encs_cmp = np.array(closest_faces.values_list('face_encoding', flat=True))
                print('enc_retrieval', time.time() - s)
                # print(short_encs_cmp.shape)
                # print(f'Retrieve: {time.time() - s}')
                ...
                face_dist = np.linalg.norm(short_encs_cmp - short_encoding, axis=1)

                # print(np.linalg.norm(short_encs[0]))
                # print(short_encs.shape, len(short_encs[0]))
                # print(np.linalg.norm(long_encs[0]))
                # print(long_encs.shape, len(long_encs[0]))
                # print(long_encs)
                ...
                if len(l2_dist) < N_COMPARISONS:
                    comparison_mat[row_num, len(l2_dist):] = l2_dist[-1] + 9
                ...
                if self.DEBUG:
                    ...
                    cmp_face_list = []
                    for cmp_face in closest_faces:
                        face_enc = cmp_face.face_encoding
                        name = cmp_face.declared_name.person_name
                        ii = Image.open(cmp_face.face_thumbnail)
                        thumbnail = ii.copy()
                        ii.close()
                        thumbnail = np.asarray(thumbnail)
                        fid = cmp_face.id
                        # date = time.strftime('%Y-%m-%d', time.localtime(cmp_face.dateTakenUTC))
                        
                        source_img_file = cmp_face.source_image_file.filename

                        row_data = {
                            'person_name': name,
                            'short_encoding': face_enc,
                            # 'date': date,
                            'face_thumbnail': thumbnail,
                            'id': fid,
                            'box_top': cmp_face.box_top,
                            'box_bottom': cmp_face.box_bottom,
                            'box_left': cmp_face.box_left,
                            'box_right': cmp_face.box_right,
                            'source_img': source_img_file
                        }
                        cmp_face_list.append(row_data)

                    roll_up_data[person.person_name] = cmp_face_list
                # print(f'Plus calc: {time.time() - s}')
                # dist_per_category.append(float(torch.min(l2_dist).detach()))
        
        if self.DEBUG:
            with open(f'/code/tmp_data/{u_img.id}.pkl', 'wb') as ph:
                pickle.dump(roll_up_data, ph)

        per_row_mins = np.min(comparison_mat, axis=1)
        best_rows = np.argsort(per_row_mins)
        top_n = best_rows[:5]
        top_n_scores = 1 - per_row_mins[top_n]
        
        # Reset the scores
        u_img.set_possibles_zero()


        best_id = self.person_ids[top_n[0]]
        first_wt = top_n_scores[0] # average_dists[most_votes_ranked[0]]
        ...
        thresh = 0.5
        print("Firstwt", first_wt, np.min(comparison_mat))
        if first_wt < thresh and self.ignore_person.id not in rejected_ids:
            ...
            person_id = person
            weight_val = 1 - first_wt # np.max((0, thresh - first_wt))
