...
class XMPFace:
    ...
    def save_file(self, filename):
        ...
        with open("/tmp/exiv2.txt", "w") as f:
            f.write("\n".join(self.cmds))
        self.cmds = []

        # Write tags with exiv2
        cmd = 'exiv2 -m "/tmp/exiv2.txt" mo "%s"' % tmpfile
        ...
        orig_px = cv2.imread(filename)
        over_px = cv2.imread(tmpfile)
        # over_px[0, 0, 0] = 0

        assert np.all(orig_px == over_px), 'Pixels changed during exiv write.'
        # Copy file back to orginal file
        copyFile(tmpfile,filename)
        orig_px2 = cv2.imread(filename)
        assert np.all(orig_px2 == over_px), 'Pixels changed during exiv copy back.'
        os.remove(tmpfile)
        # os.remove("/tmp/exiv2.txt")
        
    
if __name__ == "__main__":
    # Read tags for all .jpg files in working dir
    filename =  '/mnt/server_home/face_to_xmp/test.jpg'
    # images = []
